...
const debug = require('debug')('tr-electron-download')
...
class ElectronDownloader {
  ...
  get baseUrl () {
    return process.env.NPM_CONFIG_ELECTRON_MIRROR ||
      ...
      this.opts.mirror ||
      'https://bams-sami.int.thomsonreuters.com/artifactory/default.npm.global/UniversalContainer/tr-electron-releases/v'
  }
  ...
  get cache () {
    ...
    const oldCacheDirectory = path.join(os.homedir(), './.tr-electron')
  ...
  get filename () {
    ...
    if (this.chromedriver) {
      ...
      return `tr-electron-${suffix}-symbols.zip`
    } else if (this.dsym) {
      return `electron-${suffix}-dsym.zip`
    } else {
      return `tr-electron-${suffix}.zip`
    }
  }
  ...
  createCacheDir (cb) {
    fs.mkdirs(this.cache, (err) => {
      if (err) {
        ...
        let localCache = path.resolve('./tr-electron')
